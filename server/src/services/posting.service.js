...
export async function postDayBook(dayBookId) {
  ...
  const sales = (dayBook.sales || []).map((l, i) => {
    ...
    const discount = l.discount || 0; // per-line discount (Rs) — reduces amount AND profit
    return {
      ...l,
      discount,
      amount: l.qty * l.rate - discount, // NET of discount → reduces sale/cash/ledger too
      costRate,
      // profit = amount − cost×qty = (rate − costRate)×qty − discount (single reduction).
